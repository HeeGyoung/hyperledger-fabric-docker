...
function approveForMyOrg() {
  ...
  --ordererTLSHostnameOverride ${ORDERER}.${SWITCHWON_DOMAIN} \
...
function commitChaincodeDefinition() {
  ...
  set -x
  ./bin/peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride ${ORDERER}.${SWITCHWON_DOMAIN} \
...
function chaincodeInvoke() {
  ...
  ./bin/peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride ${ORDERER}.${SWITCHWON_DOMAIN} \
...
function chaincodeQuery() {
  ...
  infoln "Querying on peer0.userlog-org${ORG} on channel '$CHANNEL_NAME'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  # we either get a successful response, or reach MAX RETRY
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    sleep $DELAY
    infoln "Attempting to Query peer0.userlog-org${ORG}(${CORE_PEER_ADDRESS}), Retry after $DELAY seconds."
    set -x

...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o ${ORDERER}.${SWITCHWON_DOMAIN}:7050 --ordererTLSHostnameOverride ${ORDERER}.${SWITCHWON_DOMAIN} -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
